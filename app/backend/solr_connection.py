...
class SolrConnection:
    def convert_query(self, query):
        ...
        sol_query_insert = "tweet_text%3A(" + text + ")%0A"
        solr_query = "http://ec2-52-72-185-101.compute-1.amazonaws.com:8983/solr/BM25_Sentiment_Analysis_V1/select?q.op=OR&q=" + sol_query_insert + "-poi_name%3A%5B*%20TO%20*%5D&rows=50"
        data = requests.get(solr_query).json()
        dictionary_of_non_poi_responses = data['response']['docs']
        if len(dictionary_of_non_poi_responses) >= 50:
    ...
    def convert_query_for_poi(self, query):
        text = query.replace(" ", "%20")
        sol_query_insert = "tweet_text%3A(" + text + ")%0A"
        solr_query = "http://ec2-52-72-185-101.compute-1.amazonaws.com:8983/solr/BM25_Sentiment_Analysis_V1/select?q.op=OR&q=" + sol_query_insert + "AND%0Apoi_name%3A%5B*%20TO%20*%5D&rows=50"
        data = requests.get(solr_query).json()
        dictionary_of_non_poi_responses = data['response']['docs']
        if len(dictionary_of_non_poi_responses) >= 50:
            return data['response']['docs']
        ...
        solr_query = "http://ec2-52-72-185-101.compute-1.amazonaws.com:8983/solr/BM25_Sentiment_Analysis_V1/select?q.op=OR&q=" + query_final + "AND%0Apoi_name%3A%5B*%20TO%20*%5D&rows=50"
        data = requests.get(solr_query).json()
        return data['response']['docs']
